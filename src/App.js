...
const sb = {
get: async (t,q=””) => (await fetch(`${SB_URL}/${t}${q}`,{headers:H})).json(),
upsert: async (t,b) => fetch(`${SB_URL}/${t}`,{method:“POST”,headers:{…H,“Prefer”:“resolution=merge-duplicates,return=representation”},body:JSON.stringify(b)}),
patch: async (t,q,b) => fetch(`${SB_URL}/${t}${q}`,{method:“PATCH”,headers:{…H,“Prefer”:“return=representation”},body:JSON.stringify(b)}),
del: async (t,q) => fetch(`${SB_URL}/${t}${q}`,{method:“DELETE”,headers:H}),
};
...
locate:true, src:canvas.toDataURL(“image/jpeg”,0.8),
}, result => {
busy = false;
if (!activeRef.current) return;
...
<input type=“number” min=“0” value={val} onChange={e=>setVal(e.target.value)} style={{flex:1,textAlign:“center”,fontSize:100,fontWeight:800,border:“2px solid var(–color-border-secondary)”,borderRadius:24,padding:“20px 8px”,background:“var(–color-background-secondary)”,color:“var(–color-text-primary)”,minWidth:0}} autoFocus />
...
<div style={T.ph}><button onClick={onBack} style={T.backBtn}>← Back</button><p style={{...T.ptitle,fontSize:fs.xl}}>{title}</p></div>
<div style={{padding:“0 28px 80px”}}>
<label style={T.lbl}>Product Name</label><input style={T.finp} value={form.name} onChange={e=>set(“name”,e.target.value)} placeholder=“e.g. Cherry Tomatoes” />
<label style={T.lbl}>Brand</label><input style={T.finp} value={form.brand} onChange={e=>set(“brand”,e.target.value)} placeholder=“e.g. King Arthur” />
<label style={T.lbl}>SKU / Barcode</label><input style={T.finp} value={form.sku} onChange={e=>set(“sku”,e.target.value)} placeholder=“e.g. 074175604517” />
<label style={T.lbl}>Category</label>
<select style={T.finp} value={form.category} onChange={e=>set(“category”,e.target.value)}>{CATEGORIES.filter(c=>c!==“All”).map(c=><option key={c}>{c}</option>)}</select>
<label style={T.lbl}>Unit (lbs, gal, cs, btl…)</label><input style={T.finp} value={form.unit} onChange={e=>set(“unit”,e.target.value)} placeholder=“lbs” />
...
<select style={T.finp} value={form.vendor} onChange={e=>set(“vendor”,e.target.value)}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</select>
...
return (
<div style={{…T.screen,fontFamily:“system-ui,sans-serif”}}>
<div style={T.ph}><button onClick={onBack} style={T.backBtn}>← Back</button><p style={{...T.ptitle,fontSize:fs.xl}}>Confirm & Add</p></div>
<div style={{padding:“0 28px 80px”}}>
<div style={{background:”#E6F1FB”,borderRadius:20,padding:“16px 24px”,marginBottom:28}}><p style={{margin:0,fontSize:fs.sm,color:”#185FA5”,fontWeight:600}}>Found in product database — confirm details below</p></div>
<label style={T.lbl}>Product Name</label><input style={T.finp} value={form.name} onChange={e=>set(“name”,e.target.value)} />
<label style={T.lbl}>Brand</label><input style={T.finp} value={form.brand} onChange={e=>set(“brand”,e.target.value)} />
<label style={T.lbl}>SKU / Barcode</label><input style={T.finp} value={form.sku} onChange={e=>set(“sku”,e.target.value)} />
<label style={T.lbl}>Category</label>
<select style={T.finp} value={form.category} onChange={e=>set(“category”,e.target.value)}>{CATEGORIES.filter(c=>c!==“All”).map(c=><option key={c}>{c}</option>)}</select>
<label style={T.lbl}>Unit (lbs, gal, cs, btl…)</label><input style={T.finp} value={form.unit} onChange={e=>set(“unit”,e.target.value)} placeholder=“lbs” />
<label style={T.lbl}>Par Level</label><input style={T.finp} type=“number” min=“0” value={form.par} onChange={e=>set(“par”,e.target.value)} />
<label style={T.lbl}>Vendor</label>
<select style={T.finp} value={form.vendor} onChange={e=>set(“vendor”,e.target.value)}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</select>
<label style={T.lbl}>Notes</label><input style={T.finp} value={form.notes} onChange={e=>set(“notes”,e.target.value)} placeholder=“e.g. weight, size…” />
...
<input type=“number” min=“0” value={qtyVal} onChange={e=>setQtyVal(e.target.value)} style={{flex:1,textAlign:“center”,fontSize:80,fontWeight:800,border:“2px solid var(–color-border-secondary)”,borderRadius:22,padding:“16px 8px”,background:“var(–color-background-secondary)”,color:“var(–color-text-primary)”,minWidth:0}} />
...
} else { setScanResult({notFound:true,sku:code}); }
...
<ConfirmNewItemScreen initial={scanResult} vendors={vendors} onSave={item=>{onAddItem(item);setStep(“menu”);setScanResult(null);}} onBack={()=>setStep(“menu”)} />
);
return (
<div style={T.screen}>
<div style={T.ph}><button onClick={()=>setStep(“menu”)} style={T.backBtn}>← Back</button></div>
...
const saveItem = async item => { setItems(p=>p.map(i=>i.id===item.id?{…i,…item}:i)); await sb.patch(“items”,`?id=eq.${item.id}`,item); showToast(“Item updated!”); };
const addItem = async item => { const n={brand:””,notes:””,…item,id:Date.now()}; setItems(p=>[…p,n].sort((a,b)=>a.name.localeCompare(b.name))); await sb.upsert(“items”,n); showToast(“Item added!”); };
const deleteItem = async id => { setItems(p=>p.filter(i=>i.id!==id)); await sb.del(“items”,`?id=eq.${id}`); showToast(“Item deleted.”); };
const addVendor = async v => { setVendors(p=>[…p,v]); await sb.upsert(“vendors”,v); showToast(“Vendor added!”); };
const deleteVendor = async id => { setVendors(p=>p.filter(v=>v.id!==id)); setItems(p=>p.filter(i=>i.vendor!==id)); await sb.del(“items”,`?vendor=eq.${id}`); await sb.del(“vendors”,`?id=eq.${id}`); showToast(“Vendor deleted.”); };
